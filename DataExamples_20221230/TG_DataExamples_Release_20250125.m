...
param.phaseMethod     = 'generalized'; % 'hilbert' or 'generalized'
...
for iEx = 1:size(exampleSegments, 1)
    ...
    xlim(xLimit);
    ylim(yLimit);
    titleString = strcat({'sub_'}, {num2str(iSub)}, {', '}, {'session '}, {num2str(iSess)}, {', '}, brainRegionsList{iWire, 1});
    ...
    % plot
    complexFigure   = figure('units', 'centimeters', 'position', [5, 5, 4, 4]);
    c2              = cline(real(allSegPhaseData.trial{1, iSeg}), imag(allSegPhaseData.trial{1, iSeg}), [], angle(allSegPhaseData.trial{1, iSeg}));
    ...
    xline(0);
    yline(0);
    figLim          = ceil(max(abs([real(allSegPhaseData.trial{1, iSeg}), imag(allSegPhaseData.trial{1, iSeg})])) / 100) * 100;
    xlim([-figLim, figLim]);
    ylim([-figLim, figLim]);
    ...
    print(complexFigure, fullfile(paths.save, strcat(segIdx, '_ComplexPlaneExample')), '-dtiff', '-r300');
end
