...
paths.bycycleData   = 'D:\TreasureHunt\MicroBycycle_20241216'; % data from Bycycle algorithm
...
params.filterBand       = [3, 10]; % filter band for phase detection
params.phaseMethod      = 'generalized'; % generalized phase approach
params.segBlDur         = 1.5; % baseline duration in seconds before segments
params.timeBorderSTA    = 1; % time window spike-triggered average in seconds
params.nSur             = 1001;
params.condition        = {'all'; 'baseline'; 'enc_both'; 'enc_succ'; 'enc_fail'; 'rec_both'; 'rec_succ'; 'rec_fail'};

% update save path
paths.save              = fullfile(paths.save, strcat(regexprep(num2str(params.filterBand), '\s+', '_'), '_Hz_PPC_Results'));
...
parfor (iSub = 1:length(subjects), 6)
    ...
    noSprint         = []; % no SPRiNT file
    noBycycle        = []; % no Bycycle file
    ...
    for iSess = 1:size(sessions, 1)
        ...
        for iWire = 1:size(phaseChanDir, 1)
            ...
            % load phase data
            phaseDataName       = strcat('datacutFt2000HzBP_', params.phaseMethod, '_', regexprep(num2str(params.filterBand), '\s+', '_'), '.mat');
            ...
            instFreq            = (1 / (2 * pi)) * diff(unwrap(instPhase)) * phaseData.fsample;
            ...
            thetaPeakIdx        = centerFrequency >= params.filterBand(1, 1) & centerFrequency <= params.filterBand(1, 2); % extract only peaks in theta band
            ...
            bBurst                          = [];
            try
                bycycleData                     = load(fullfile(paths.bycycleData, subjects{iSub}, sessions(iSess).name, mwChanDir(iWire).name, ...
                    strcat('datacutFt2000HzBP_bycycle_', num2str(params.filterBand(1)), '_', num2str(params.filterBand(2)), '.mat')));
                bBurst                          = double(bycycleData.bBurst);  
                bBurst(artifactData.bArtifact)  = NaN;

            catch
                noBycycle       = cat(1, noBycycle, [iSub, iSess, iWire]); % bookkeeping
                fprintf('No bycycle file for %s, session %d, channel %d', subjects{iSub}, iSess, iWire);
            end
            ...
            try
                c4a = load(fullfile(mwChanPath, params.c4aName{1, 1}));
            catch
                c4a = load(fullfile(mwChanPath, params.c4aName{1, 2}));
            end
            ...
            for iClus = 1:max(t.cluster_class(:, 1))
                ...
                for iCond = 1:size(params.condition, 1)
                    ...
                    % segment index
                    if contains(params.condition{iCond}, 'all')
                        condDataOrigIdx = true(size(instPhase));
                        bGoodMemSeg     = true;
                    elseif contains(params.condition{iCond}, 'baseline')
                        condDataOrigIdx = blSegmentDataIdx;
                        bGoodMemSeg     = true(max(blSegmentDataIdx), 1);
                    elseif contains(params.condition{iCond}, 'enc')
                        condDataOrigIdx = encSegmentDataIdx;
                        bGoodMem        = bGoodMemEnc;
                    elseif contains(params.condition{iCond}, 'rec')
                        condDataOrigIdx = recSegmentDataIdx;
                        bGoodMem        = bGoodMemRec;
                    end

                    % memory performance index
                    if contains(params.condition{iCond}, 'both')
                        bGoodMemSeg = true(size(bGoodMem, 1), 1);
                    elseif contains(params.condition{iCond}, 'succ')
                        bGoodMemSeg = bGoodMem;
                    elseif contains(params.condition{iCond}, 'fail')
                        bGoodMemSeg = ~bGoodMem;
                    end
                    ...
                    if ~isempty(catPhases)
                        segPPC                  = TG_PPC_20241128(catPhases);
                    else
                        segPPC                  = NaN;
                    end

                    % get surrogate MRV
                    surSegMrv       = NaN(1, params.nSur);
                    surSegPPC       = NaN(1, params.nSur);
                    for iSur = 1:params.nSur
                        ...
                        % get surrogate mean resultant vector and PPC
                        try
                            [surSegMrv(:, iSur), ~]     = circ_axialmean(surCatPhases, [], 1);
                            surSegPPC(:, iSur)          = TG_PPC_20241128(surCatPhases);
                    ...
                    segRank             = sum(segPPC > surSegPPC, 2) / size(surSegPPC, 2);
                    ...
                    segSpikeFreq        = cell(size(thisSegments, 1), 1);
                    ...
                    segBurstIdx         = cell(size(thisSegments, 1), 1);
                    ...
                    for iSeg = 1:size(thisSegments, 1)
                        ...
                        % get frequency
                        segSpikeFreq{iSeg, 1}           = instFreq(segSpikeSamples{iSeg, 1})';
                        ...
                        % get theta burst index
                        try
                            segBurstIdx{iSeg, 1}        = bBurst(segSpikeSamples{iSeg, 1})';
                        catch
                            segBurstIdx{iSeg, 1}        = zeros(size(segSpikeSamples{iSeg, 1}))';
                        end
                        ...
                        if contains(params.condition{iCond}, {'enc', 'rec'})
                            segBlSmpl           = params.segBlDur * phaseData.fsample;
                            segBlSpikeIdx       = find(thisClusSamplesOrig >= (find(bThisSegment, 1, 'first') - segBlSmpl) & thisClusSamplesOrig <= find(bThisSegment, 1, 'first'));
                            segBlFr(iSeg, 1)    = size(segBlSpikeIdx, 1) / params.segBlDur;
                        else
                            segBlFr(iSeg, 1)    = NaN;
                        end
                    end
                    ...
                    if contains(params.condition{iCond}, {'enc', 'rec'})
                        ...
                        % get sample window
                        sampleWindowSTA    = -params.timeBorderSTA * LFPdata.fsample:1:params.timeBorderSTA * LFPdata.fsample;
                    ...
                    if strcmp(params.condition{iCond}, 'all')
                        allDur              = segDur;
                        allFr               = segFr;
                        ...
                        allBurstIdx         = segBurstIdx;
                        allSpikeSlope       = segSpikeSlope;
                        allSlopeIdx         = segSlopeIdx;
                        allPPC              = segPPC;
                        allMrv              = segMrv;
                        allRank             = segRank;
                    elseif strcmp(params.condition{iCond}, 'baseline')
                        blDur               = segDur;
                        blFr                = segFr;
                        ...
                        blSpikeFreq         = segSpikeFreq;
                        blPPC               = segPPC;
                        blMrv               = segMrv;
                        blRank              = segRank;
                    elseif strcmp(params.condition{iCond}, 'enc_both')
                        encDur              = segDur;
                        encFr               = segFr;
                        encBlFr             = segBlFr;
                        ...
                        encSpikeFreq        = segSpikeFreq;
                        encPowerIdx         = segPowerIdx;
                        encThetaIdx         = segThetaIdx;
                        encBurstIdx         = segBurstIdx;
                        ...
                        encBothRank         = segRank;
                        encBothSTA          = STA;
                    elseif strcmp(params.condition{iCond}, 'enc_succ')
                        encSuccRank         = segRank;
                        encSuccSTA          = STA;
                    elseif strcmp(params.condition{iCond}, 'enc_fail')
                        encFailRank         = segRank;
                        encFailSTA          = STA;
                    elseif strcmp(params.condition{iCond}, 'rec_both')
                        recDur              = segDur;
                        recFr               = segFr;
                        recBlFr             = segBlFr;
                        ...
                        recSpikeFreq        = segSpikeFreq;
                        recPowerIdx         = segPowerIdx;
                        recThetaIdx         = segThetaIdx;
                        recBurstIdx         = segBurstIdx;
                        ...
                        recBothRank         = segRank;
                        recBothSTA          = STA;
                    elseif strcmp(params.condition{iCond}, 'rec_succ')
                        recSuccRank         = segRank;
                        recSuccSTA          = STA;
                    elseif strcmp(params.condition{iCond}, 'rec_fail')
                        recFailRank         = segRank;
                        recFailSTA          = STA;
                    end
                end
                ...
                unitRes.encSpikeFreq        = encSpikeFreq;
                ...
                unitRes.encBurstIdx         = encBurstIdx;
                unitRes.encBothPPC          = encBothPPC;
                ...
                unitRes.recSpikeFreq        = recSpikeFreq;
                ...
                unitRes.recBurstIdx         = recBurstIdx;
                unitRes.recBothPPC          = recBothPPC;
...
allNoBycycle            = cat(1, allNoBycycle{:});
...
STAcfg.bpfreq           = s.params.filterBand;
STAcfg.demean           = 'yes';

%% define variables

% time axis for STA
timeAxisSTA      = linspace(-s.params.timeBorderSTA, s.params.timeBorderSTA, size(r.allRes(1).encBothSTA, 2));
...
for iCond = 1:size(conditions, 1)
    ...
    parfor iCell = 1:size(r.allRes, 1)
        ...
        % surrogate tests
        surWwF    = zeros(parforRes.params.nSur, 1);
        for iSur = 1:parforRes.params.nSur
        ...
        wwRank      = sum(wwF > surWwF) / parforRes.params.nSur;
        ...
        encP(encP < (1 / parforRes.params.nSur))     = 1 / parforRes.params.nSur;
        recP(recP < (1 / parforRes.params.nSur))     = 1 / parforRes.params.nSur;
        wwP(wwP < (1 / parforRes.params.nSur))       = 1 / parforRes.params.nSur;
        ...
        encHistc = histcounts(cellEncPhase, parforRes.params.polarHistEdges);
        encHistg = polarhistogram('BinEdges', parforRes.params.polarHistEdges, 'BinCounts', encHistc, 'FaceColor', 'k');
        hold on;
        polarplot([cellEncMean, cellEncMean], [0, max(encHistc)], 'r', 'LineWidth', 2);
        set(gca, 'ThetaTickLabel', parforRes.params.polarHistLabels);
        rlim([0, max(encHistc)]);
        ...
        recHistc = histcounts(cellRecPhase, parforRes.params.polarHistEdges);
        recHistg = polarhistogram('BinEdges', parforRes.params.polarHistEdges, 'BinCounts', recHistc, 'FaceColor', 'k');
        hold on;
        ...
        if ~isfolder(fullfile(paths.save, 'UnitFigures'))
            mkdir(fullfile(paths.save, 'UnitFigures'));
        end
...
for iCell = 1:size(phaseRes, 1)
    ...
    objectCellRank(iCell, 1)    = TG_PermTest1D_2PS_20230727(phaseRes(iCell).encFr, phaseRes(iCell).encBlFr, parforRes.params.nSur, s.randSeedNum);
