...
paths.phaseRes      = 'D:\TreasureHunt\PhaseAnalysis_20230921\1_10_Hz_PPC_Results'; % results from phase analysis
paths.save          = 'D:\TreasureHunt\SpikeFieldCoupling_20231123\results_20250131'; % save folder
...
params.timeBorder   = 0.5; % time before and after spike for spike-field coherence in seconds
params.sfcFs        = 250; % sampling rate for SFC
params.nSubSmp      = 100; % number of subsamples
    ...
    for iSess = 1:size(sessions, 1)
        ...
        for iWire = 1:size(mwChanDir, 1)
            ...
            for iClus = 1:max(t.cluster_class(:, 1))
                ...
                smpOffsetST         = (params.timeBorder * LFPdata.fsample);
                ...
                % downsample
                rsmpCfg             = [];
                rsmpCfg.resamplefs  = params.sfcFs;
                ...
                %% subsampling
                encSuccSFC          = nan(params.nSubSmp, size(specCfg.foi, 2));
                encFailSFC          = nan(params.nSubSmp, size(specCfg.foi, 2));
                recSuccSFC          = nan(params.nSubSmp, size(specCfg.foi, 2));
                recFailSFC          = nan(params.nSubSmp, size(specCfg.foi, 2));
                for iSmp = 1:params.nSubSmp
...
%% results across brain regions

% collect brain regions
allBrainRegIdx          = {incRes.brainRegion}';
...
for iReg = 1:size(brainRegion, 1)
    ...
    % encoding
    regEncSucc                  = encSucc(regIdx, :);
    regEncFail                  = encFail(regIdx, :);

    % retrieval
    regRecSucc                  = recSucc(regIdx, :);
    regRecFail                  = recFail(regIdx, :);

    % plot - encoding
    encRegFigSFC = figure;
    TG_ShadeSEM_20210714(incRes(1).foi, regEncSucc * 100, [0.1, 0.6, 0.1], 0.4);
    hold on;
    TG_ShadeSEM_20210714(incRes(1).foi, regEncFail * 100, [0.6, 0.1, 0.1], 0.4);
    xlabel('Frequency (Hz)');
    xlim([0, 100]);
    set(gca, 'XScale', 'log', 'tickDir', 'out');
    box off;
    ylabel('SFC (%)');
    ylim([0, 5]);
    title(strcat('Encoding:', brainRegion{iReg}));

    % print figure
    print(encRegFigSFC, fullfile(paths.save, strcat('encodingSFC_', brainRegion{iReg})), '-dsvg', '-r300');

    % plot - retrieval
    recRegFigSFC = figure;
    TG_ShadeSEM_20210714(incRes(1).foi, regRecSucc * 100, [0.1, 0.6, 0.1], 0.4);
    hold on;
    TG_ShadeSEM_20210714(incRes(1).foi, regRecFail * 100, [0.6, 0.1, 0.1], 0.4);
    xlabel('Frequency (Hz)');
    xlim([0, 100]);
    set(gca, 'XScale', 'log', 'tickDir', 'out');
    box off;
    ylabel('SFC (%)');
    ylim([0, 5]);
    title(strcat('Retrieval:', brainRegion{iReg}));

    % print figure
    print(recRegFigSFC, fullfile(paths.save, strcat('recallSFC_', brainRegion{iReg})), '-dsvg', '-r300');
end
